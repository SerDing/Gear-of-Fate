...
local _Tile = require("system.scene.objects.tile")
local _AnimObj = require("system.scene.objects.animobj")
...
function _Level:Ctor(path, LEVELMGR) --initialize
    ...
    self.map = _RESOURCE.LoadData(path)
...
function _Level:LoadTiles()
    ...
    if self.map["[extended tile]"] then
        ...
        for i,v in ipairs(self.map["[extended tile]"]["[tile map]"]) do
            ...
            self.extendedTiles[i]:SetPos(((i - 1) % n) * 224, math.floor(i / n) * self.extendedTiles[i].sprite:GetHeight() - 1)
...
---@param cx number @camera x
---@param cy number @camera y
function _Level:Draw(cx, cy)

    self:DrawBackGround("[far]", "[far sight scroll]", cx)
    self:DrawBackGround("[mid]", "[middle sight scroll]", cx)
    self:DrawTile(cx, cy)
	self:DrawExtendedTile(cx, cy)

    self:DrawLayer("[bottom]", cx, cy)
    self:DrawLayer("[closeback]", cx, cy)
...
---@param scroll string
---@param cx number
function _Level:DrawBackGround(type, scroll, cx)
    --print("cx", cx)
    if self.map["[background animation]"] then
        local name = self.map["[background animation]"][type]["[filename]"]
        local bganim = self.res[type][name]

        --Parallax Scrolling: camera move to right, background move to left, the farther one moves slower.
        --Camera of level has translate graphics to left by some distance(include background)
        --So background needs to be move some distance to right.
        --Finally, the position of background is the distance background moves relative to camera
        local startX = cx * (100 - self.map[scroll]) / 100

        local y = self.map["[background pos]"] or 80
        local bgWidth = bganim:GetWidth()
        --startX = (startX % -bgWidth)

        local leftBoundX = cx - bgWidth
        while startX <= leftBoundX do
            startX = startX + bgWidth
        end
        local cameraRightSideX = cx + _GRAPHICS.GetWidth()
        while startX < cameraRightSideX do
            bganim.sprite:SetRenderValue("position", startX, y)
            bganim:Draw()
            startX = startX + bgWidth
		end
    end
...
---@param cx number
---@param cy number
function _Level:DrawTile(cx, cy)
    -- horizental
    local index = Floor(Abs(cx) / 224) + 1 -- plus 1 because of floor() maybe return 0
	local num = Ceil(_GRAPHICS.GetWidth() / 224)
...
---@param cx number
---@param ey number
function _Level:DrawExtendedTile(cx, cy)
	...
	-- local num = math.ceil(_GRAPHICS.GetWidth() / 224) -- 向上取整
    ...
    -- local YEnd = math.ceil((ey + _GRAPHICS.GetHeight() - (480 + 80)) / 120) --#self.extendedTiles / tileNum、
...
---@param cx number
---@param cy number
function _Level:DrawLayer(layer, cx, cy)
    for n=1,#self.layers[layer] do
        self.layers[layer][n]:Draw(cx, cy)
    end
end
