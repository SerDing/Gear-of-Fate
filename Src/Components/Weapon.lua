...
function _Weapon:SetEquID(id) -- set equipment id
	...
	if _equipment:GetType() == "weapon" and _equipment:LayerNum() == 4 then
		self.actor_.avatar:GetWidget("weapon_b2"):SetActive(not bool)
		self.actor_.avatar:GetWidget("weapon_c2"):SetActive(not bool)
	end
end
...
function _Weapon:SetRes(weaponType, filePathArg)
	...
	if self.single then
		if love.filesystem.exists(_RESMGR.pathHead .. _path_b) then
			self.actor_.avatar:GetWidget("weapon_b1"):SetImgPathArg(_path_b)
		elseif love.filesystem.exists(_RESMGR.pathHead .. _path_c) then
			self.actor_.avatar:GetWidget("weapon_c1"):SetImgPathArg(_path_c)
		else
			print(_path_b)
			print(_path_c)
	...
	self.actor_.avatar:GetWidget("weapon_b2"):SetActive(false)
	self.actor_.avatar:GetWidget("weapon_c2"):SetActive(false)
end
...
function _Weapon:SetSingle(bool)
	self.single = bool
	self.actor_.avatar:GetWidget("weapon_b1"):SetActive(not bool)
end
