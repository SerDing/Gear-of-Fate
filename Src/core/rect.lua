...
local _Rect = require("core.class")() ---@class Rect

function _Rect:Ctor(x, y, w, h) --initialize
	self.posistion = { x = 0.0, y = 0.0}
	self.size = {w = 0, h = 0}
	self.scale = {x = 1, y = 1}
	...
	self.posistion.x = x or 0.0
	self.posistion.y = y or 0.0
	self.size.w = w or 0
	self.size.h = h or 0
	self.dir = 1
	...
	self:SetDrawType("fill")
	self:Update()
end

function _Rect:Update()
	self.x1 = self.posistion.x - self.cenPos.x * self.scale.x * self.dir
	self.y1 = self.posistion.y - self.cenPos.y * self.scale.y
	self.x2 = self.x1 + self.size.w * self.scale.x * self.dir
	self.y2 = self.y1 + self.size.h * self.scale.y
end

function _Rect:Draw(x, y, rotation, sx, sy)
